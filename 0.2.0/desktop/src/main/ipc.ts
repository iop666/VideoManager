...
import { applyRename, buildRenamePreview, isUndoAvailable, undoRename } from './services/rename'
...
export function registerIpcHandlers(taskQueue: TaskQueue): void {
  ...
  ipcMain.handle('settings:page-size', (_e, value?: number): number => {
    if (value !== undefined && value > 0) setSetting('page_size', String(value))
    return Number(getSetting('page_size') ?? '50')
  })
  ...
  ipcMain.handle('server:stop', (): void => stopHttpServer())
