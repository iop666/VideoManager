...
export function exportVideosMeta(): VideoMetaExport[] {
  ...
  const rows = db
    .prepare(
      `SELECT v.sha256, v.title, v.file_name, v.category_id, c.name AS category, c.color AS category_color,
              v.author_id, a.name AS author, a.color AS author_color, v.rating, v.remark, v.is_favorite,
              v.file_size, v.duration, v.width, v.height, v.codec, v.format, v.play_count,
...
export function importVideosMeta(
  ...
  for (const entry of items) {
    ...
    if (!row) {
      ...
      if (placeholder) {
        ...
        try {
          const ins = db
            .prepare(
              `INSERT OR IGNORE INTO videos (title, file_path, file_name, file_size, duration, width, height,
    ...
    db.prepare(
      'UPDATE videos SET title = ?, category_id = ?, author_id = ?, rating = ?, remark = ?, is_favorite = ? WHERE id = ?'
    ).run(
      item.title || '(未命名)',
      categoryId,
      authorId,
