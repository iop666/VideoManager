...
onMounted(async () => {
  ...
  coverMode.value = (await window.api.getCoverMode()) as 'landscape' | 'normal'
  await refreshServerStatus()
  await loadBackup()
})

async function onCoverModeChange(v: 'landscape' | 'normal'): Promise<void> {
  coverMode.value = await (window.api.setCoverMode(v) as Promise<string>) as 'landscape' | 'normal'
  ...
  <div class="page">
    ...
    <n-card title="显示数据" size="small" style="max-width: 720px; margin-top: 16px">
      ...
      <div class="appearance-row" style="margin-top: 12px">
        ...
        <n-radio-group :value="coverMode" @update:value="onCoverModeChange">
