  ...
  if (existing) {
    db.prepare(
      `UPDATE videos SET title = ?, file_size = ?, duration = ?, width = ?, height = ?,
       codec = ?, audio_codec = ?, format = ?, fps = COALESCE(?, fps), status = 'ready', date_modified = ?
       WHERE id = ?`
    ).run(title, fileStat.size, info.duration, info.width, info.height, info.videoCodec,
      info.audioCodec, options.format, info.fps, new Date(fileStat.mtimeMs).toISOString(), existing.id)
    newId = existing.id
  } else {
    const res = db
      .prepare(
        `INSERT INTO videos (title, file_path, file_name, more_file_names, file_size, duration, fps, width, height,
                             codec, audio_codec, format, date_modified, status)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'ready')`
      )
      .run(
        title, outPath, basename(outPath), JSON.stringify([basename(outPath)]), fileStat.size,
        info.duration, info.fps, info.width, info.height, info.videoCodec, info.audioCodec, options.format,
