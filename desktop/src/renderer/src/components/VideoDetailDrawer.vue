...
/** 本地位置：缺失/占位显示「无本地文件」，其余显示真实路径 */
const displayPath = computed<string>(() => {
  const v = video.value
  if (!v) return '无本地文件'
  const p = v.filePath ?? ''
  if (!p || p.startsWith('restored://') || v.status === 'missing') return '无本地文件'
  return p
})
      ...
      <n-spin :show="loading">
        <template v-if="video">
          ...
          <div class="file-name-block" style="margin-top: 14px">
            <span class="wrap-text file-name">{{ video.fileName || moreFileNames[0] || '（未记录文件名）' }}</span>
          ...
          <!-- 本地位置（小号等宽，参考元数据编辑页样式） -->
          <div class="meta-local" style="margin-top: 10px">
            本地位置：{{ displayPath }}
          </div>
          ...
          <div class="file-names" style="margin-top: 10px">
            <div class="file-names-title">文件名记录</div>
...
.file-names .meta-local + .meta-local {
  margin-top: 2px;
}
