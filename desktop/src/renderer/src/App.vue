...
onMounted(() => {
  ...
  // 预加载显示设置（每页数量/封面模式）与元数据排序配置
  void appStore.loadDisplaySettings()
...
const menuOptions: MenuOption[] = [
  {
    ...
    children: [
      ...
      { label: '标签管理', key: '/stats', icon: renderIcon(BarChartOutline) }
    ]
  },
  {
    label: '工具',
...
const navGroups = [
  {
    ...
    items: [
      ...
      { label: '标签管理', key: '/stats', icon: BarChartOutline }
    ]
  },
  {
    label: '工具',
...
function handleMenuSelect(key: string): void {
  router.push(key)
  // 导航到视频库/元数据时发送刷新信号（点击当前页导航也触发）
  if (key === '/library' || key === '/metadata') {
    appStore.bumpNavTick()
  }
}
            ...
            <n-layout>
              ...
              <n-layout-content :native-scrollbar="false" class="content">
                <router-view v-slot="{ Component }">
                  <keep-alive :include="['MetadataView', 'LibraryView']">
